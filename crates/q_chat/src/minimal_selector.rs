...
/// A simple selector for minimal builds using crossterm
...
/// Select profile using minimal selector
pub fn select_profile_with_minimal_selector(context_manager: &ContextManager) -> Result<Option<String>> {
    ...
    let mut selector = MinimalSelector::new(&profiles, "Select profile:", false);
    let result = selector.run()?;
    
    Ok(result.and_then(|v| v.into_iter().next()))
}

/// Select files using minimal selector
pub fn select_files_with_minimal_selector() -> Result<Option<Vec<String>>> {
    ...
    let mut selector = MinimalSelector::new(&files, "Select files:", true);
    selector.run()
}

/// Select context paths using minimal selector
pub fn select_context_paths_with_minimal_selector(context_manager: &ContextManager) -> Result<Option<(Vec<String>, bool)>> {
    ...
    let mut selector = MinimalSelector::new(&display_paths, "Select paths to remove:", true);
    let result = selector.run()?;
...
/// Select command using minimal selector
pub fn select_command_with_minimal_selector(context_manager: &ContextManager, tools: &[String]) -> Result<Option<String>> {
    ...
    let mut selector = MinimalSelector::new(&commands, "Select command:", false);
    let result = selector.run()?;
        ...
        if !selections.is_empty() {
            ...
            match CommandType::from_str(selected_command) {
                Some(CommandType::ContextAdd(cmd)) => {
                    ...
                    match select_files_with_minimal_selector()? {
                ...
                Some(CommandType::ContextRemove(cmd)) => {
                    ...
                    match select_context_paths_with_minimal_selector(context_manager)? {
                ...
                Some(CommandType::Tool(cmd)) => {
                    ...
                    let mut selector = MinimalSelector::new(tools, "Select tool:", false);
                    match selector.run()? {
